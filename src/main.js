...
const updateDHISEvents = (eventsUpdate) => {
...
const updateTrackedEntityInstances = (trackedEntityInstances) => {
    return trackedEntityInstances.map(trackedEntityInstance => {
        const options = {
            method: 'PUT',
            uri: `${ENROLLMENT_URL}/${trackedEntityInstance['trackedEntityInstance']}`,
            body: trackedEntityInstance,
            json: true
        };
        return rq(options);
    });
...
const pullMapping = async (minimum) => {
            ...
            case 'mysql':
                if (queryFile && fileExists(queryFile)) {
                    ...
                    try {
                        ...
                        winston.log('error', 'Mysql not found or query not OK' + JSON.stringify(e));
                    }
                } else {
    ...
    try {
        if (trackedEntityInstancesUpdate.length > 0) {
            const instancesResults = await updateTrackedEntityInstances(trackedEntityInstancesUpdate);
            processResponses(instancesResults, 'Tracked entity instance');
        }
    } catch (e) {
        processResponse(e, 'Tracked entity instance');
    }
    ...
    try {
        if (eventsUpdate.length > 0) {
            const eventsResults = await updateDHISEvents(eventsUpdate);
            processResponses(eventsResults, 'Event');
        }
    } catch (e) {
        processResponse(e, 'Event');
    }
