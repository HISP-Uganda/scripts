...
const findCode = facility => {
    facility = facility.toLowerCase();
    if (facility.indexOf('tororo') !== -1) {
        return '039';
    } else if (facility.indexOf('jinja') !== -1) {
        return '004';
    } else if (facility.indexOf('mubende') !== -1) {
        return '037';
    } else if (facility.indexOf('arua') !== -1) {
        return '048';
    } else if (facility.indexOf('apac') !== -1) {
        return '017';
    } else if (facility.indexOf('kabale') !== -1) {
        return '168';
    }
    return '1test';
};
        ...
        switch (dataType) {
            case 'mysql':
                if (queryFile && fileExists(queryFile)) {
                    ...
                    try {
                        let found = await readMysql(dataURL, sql, [minimum, moment().format('YYYY-MM-DD HH:mm:ss')]);
                        found = found.map(f => {
                            const date = moment(f['sample_collection_date']).format('MM/YYYY');
                            const facility = findCode(f['department']);
                            const number = f['registration_number'] || f['study_number'];
                            f['registration_number'] = `${facility}/${number}/${date}`;
                            return f;
                        });
                        ...
                        _.forOwn(results, (v, k) => {
                            let i = 1;
                            let currentData = _.groupBy(v, 'organism');
